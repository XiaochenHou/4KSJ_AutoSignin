...
# global variables
uname = 'houxiaochen01@gmail.com'
upassword = 'Hz6wjnN7WXED@ir'
...
def login(uname, upassword):
    ...
    data = {'formhash': formhash, 'referer': 'https://www.4ksj.com/', 'username': uname, 'password': upassword,
            'questionid': '0', 'answer': ''}
    ret = r.post(
        'https://www.4ksj.com/member.php?mod=logging&action=login&loginsubmit=yes&loginhash=' + loginhash + '&inajax=1',
...
def qiandao():
    ...
    # 这里需要改一下：------------------------------------2022-12-15------------------------------------------------
    ret = r.get('https://www.4ksj.com/qiandao.php?mod=sign&operation=list&inajax=1&ajaxtarget=ranklist',
                headers=headers).text
    # print(ret)
    time.sleep(1)
    ret = r.get(
        'https://www.4ksj.com/qiandao.php?mod=sign&operation=qiandao&formhash=' + formhash + '&format=empty&inajax=1&ajaxtarget=',
        headers=headers).text
    # print(ret)
    ret = r.get('https://www.4ksj.com/qiandao.php?sign=' + formhash, headers=headers).text
    time.sleep(1)
    # print(ret)
    if '您今天已经打过卡了，请勿重复操作！' in ret:
        print('签到结果：今日已签')
